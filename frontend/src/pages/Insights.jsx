...
export default function Insights() {
  ...
  if (!insights || (insights.currentMonthTotal === 0 && insights.lastMonthTotal === 0)) {
    return (
      <div className="p-8 max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Smart Insights</h1>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mt-16 p-8 bg-white rounded-lg shadow-sm">
          <LightBulbIcon className="mx-auto h-12 w-12 text-gray-400" />
          <h3 className="mt-2 text-lg font-medium text-gray-900">Not Enough Data Yet</h3>
          <p className="mt-1 text-sm text-gray-500">
            Keep tracking your expenses, and we'll generate personalized insights for you here!
          </p>
        </motion.div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="card mb-6">
      ...
      {insights.anomalies.length > 0 && (
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="card mb-6">
      ...
      {insights.recommendations.length > 0 && (
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="card">
           <div className="flex items-center gap-3 mb-4"><LightBulbIcon className="h-6 w-6 text-primary" /><h2 className="text-xl font-semibold">Budget Suggestions for Next Month</h2></div>
          <div className="space-y-3">
